...
use crate::{
	...
	task::{
		strategy::{
			do_post_recovery_check, is_unavailable, N_PARALLEL, REGULAR_CHUNKS_REQ_RETRY_LIMIT,
		},
...
use polkadot_primitives::{AuthorityDiscoveryId, ValidatorIndex};
...
pub struct FetchChunks {
	...
	requesting_chunks: FuturesUndead<(
	...
	async fn run(
		...
		if !common_params.bypass_availability_store {
			...
			self.validators.retain(|validator_index| {
				local_chunk_indices
					.iter()
					.find(|(v_index, _)| v_index == validator_index)
					.is_none()
			});
		}
		...
		self.validators.retain(|v_index| {
			state.received_chunks.values().find(|c| v_index == &c.validator_index).is_none() &&
